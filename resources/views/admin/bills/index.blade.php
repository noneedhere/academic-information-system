<x-layouts.app :title="'Bill Management'">
    ...
    {{-- Filters --}}
    <x-card class="mb-6">
        <form method="GET" action="{{ route('admin.bills.index') }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    @forelse ($bills as $bill)
                        <tr>
                            <td>
                                ...
                                @if ($bill->description)
                                    <p class="text-xs text-gray-400 mt-0.5 max-w-xs truncate">{{ $bill->description }}</p>
                                @endif
                            </td>
                            <td>
                                <div class="flex items-center gap-2">
                                    <img src="{{ $bill->student->profile_photo_url }}" alt="{{ $bill->student->name }}"
                            ...
                            <td>
                                <div class="flex items-center justify-end gap-2">
                                    {{-- Toggle Status --}}
                                    <form method="POST" action="{{ route('admin.bills.toggle-status', $bill) }}" class="inline">
                                        @csrf
                                        @method('PATCH')
                                        <button type="submit" class="btn-ghost btn-sm {{ $bill->status === App\Enums\BillStatus::Paid ? 'text-amber-600 hover:bg-amber-50' : 'text-emerald-600 hover:bg-emerald-50' }}"
                                    ...
                                    {{-- Edit --}}
                                    <a href="{{ route('admin.bills.edit', $bill) }}" class="btn-ghost btn-sm" title="Edit">
                                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="text-center py-12">
                                ...
                                <p class="text-sm text-gray-400 mt-1">Try adjusting your filters or create a new bill.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
